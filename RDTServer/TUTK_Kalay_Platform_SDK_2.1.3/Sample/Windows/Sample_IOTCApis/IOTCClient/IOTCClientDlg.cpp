...
DWORD WINAPI ClientThread(void* arg)
{
	...
	if(Sinfo.Mode ==0)
		...
		sprintf(buf, "Connected!! from %s:%d[%s] Mode=LAN\r\n", Sinfo.RemoteIP, Sinfo.RemotePort, Sinfo.UID);
...
void Cp2pClientDlg::OnBnClickedButton2()
{
	...
	ret = IOTC_Initialize2(atoi(mUDPPort));
	...
	if (mSID < 0)
	{
		sprintf(buf,"IOTC_Get_SessionID error code[%d]\r\n", mSID);
		p->mDebugPrint = p->mDebugPrint + buf;
		setTimer(p->m_hWnd, 1, 100, NULL);
		return 0;
	}
	ret = IOTC_Connect_ByUID_Parallel(mUID.GetBuffer(), mSID);
	sprintf(buf, "P2P_Connect_ByUID_Parallel() ret = %d\r\n", ret);
	mDebugPrint = mDebugPrint + buf;
	this->UpdateData(false);
	if (ret < 0)
		return;
