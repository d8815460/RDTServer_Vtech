...
int main(int argc, char *argv[])
{
	...
	struct st_SearchDeviceInfo psSearchDeviceInfo[10];
	IOTC_Search_Device_Start(SEARCH_DEVICE_TIME, SEARCH_DEVICE_TIME_ONE_ROUND);
	while(1)
	{
		int i;
		sleep(1);
		int nRet =IOTC_Search_Device_Result(psSearchDeviceInfo, 10, 0);
		if(nRet < 0)
		{
			break;
		}
		for(i=0;i<nRet;i++)
		{
			printf("UID[%s] Addr[%s:%d]\n", psSearchDeviceInfo[i].UID, psSearchDeviceInfo[i].IP, psSearchDeviceInfo[i].port);
		}
	}
